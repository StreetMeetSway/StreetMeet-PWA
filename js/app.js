...
SM.submitThread = function(communityId) {
  ...
  db.collection('communities').doc(communityId)
    ...
    .then(function(docRef) {
      ...
      /* Compress to ≤ 300KB */
      SM.compressImage(imageFile, 1200, 0.82).then(function(compressed) {
        var storagePath = 'board/' + communityId + '/' + docRef.id + '/image.jpg';
        var storageRef  = firebase.storage().ref(storagePath);

        return storageRef.put(compressed).then(function() {
...
SM.openThread = function(communityId, threadId) {
  ...
  db.collection('communities').doc(communityId)
    ...
    .then(function(doc) {
      ...
      /* Thread image — shown below body text if present */
      if (t.imageURL) {
        var imgDiv = document.createElement('div');
        imgDiv.style.cssText = 'margin-top:12px;';
        var img = document.createElement('img');
        img.src = t.imageURL;
        img.alt = t.title || 'Thread image';
        img.style.cssText = 'max-width:100%;display:block;border:1px solid var(--gray-200);cursor:pointer;';
        img.addEventListener('click', function() {
          SM.openLightbox([t.imageURL], 0);
        });
