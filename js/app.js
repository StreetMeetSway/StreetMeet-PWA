...
SM._renderHostCards = function(container, communityId, docs) {
  ...
  if (docs && docs.length > 0) {
    ...
    docs.forEach(function(doc) {
      ...
      roleTag.style.cssText = 'font-family:var(--font-head);font-size:0.6rem;letter-spacing:0.1em;padding:2px 6px;border-radius:2px;margin-left:6px;vertical-align:middle;';
      if (u.role === 'admin') {
        roleTag.style.background = 'var(--black)';
        roleTag.style.color = 'var(--white)';
        roleTag.textContent = 'ADMIN';
      } else {
        roleTag.style.background = 'var(--red)';
        roleTag.style.color = 'var(--white)';
  ...
  db.collection('communities').doc(communityId)
    ...
    .then(function(snap) {
      ...
      snap.forEach(function(doc) {
        ...
        /* Build reply element using DOM API — avoids apostrophe-in-onclick bugs */
        var replyDiv = document.createElement('div');
        replyDiv.className = 'board-reply';
        ...
        var bodyDiv = document.createElement('div');
        bodyDiv.className = 'comment-body';
        if (quoteHTML) bodyDiv.innerHTML = quoteHTML;
        ...
        /* Reply trigger — data attrs, addEventListener, no inline string */
