    ...
    el.innerHTML =
      ...
      '<div style="margin-top:40px">' +
        ...
        '<div style="display:flex;flex-direction:column;gap:8px;max-width:520px" id="admin-communities-list">' +
          SM._communities.map(function(c) {
            var isLocked = ['smdc','smwa','smmd'].indexOf(c.id) > -1;
            var isActive = c.active !== false;
            return '<div style="display:flex;align-items:center;justify-content:space-between;' +
              'padding:12px 16px;background:var(--white);border:1px solid var(--gray-200);">' +
              '<div>' +
                '<span style="font-family:var(--font-head);font-size:1.1rem;letter-spacing:0.06em">' + c.code + '</span>' +
              ...
              '<div style="display:flex;gap:8px">' +
                (isLocked ?
                  ...
                  : isActive ?
                    '<button class="action-btn remove" onclick="SM.removeCommunity(\'' + c.id + '\',\'' + c.code + '\')">REMOVE</button>'
                    :
                    '<button class="action-btn promote" onclick="SM.restoreCommunity(\'' + c.id + '\',\'' + c.code + '\')">RESTORE</button>'
                ) +
              '</div>' +
            '</div>';
          }).join('') +
