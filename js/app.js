...
SM.updateNav = function(activePageId) {
  ...
  var communityItems = SM._communities.length
    ? SM._communities.map(function(c) {
        return '<a class="nav-dropdown-item" onclick="SM.navGo(\'' + c.id + '\')">' + c.code + ' — ' + c.name + '</a>';
      }).join('')
...
SM.loadCommunities = function() {
  return db.collection('communities').orderBy('createdAt', 'asc').get()
    .then(function(snap) {
      ...
      SM._communities = snap.docs.map(function(doc) {
      ...
      '<div class="section" style="padding-top:var(--space-xl)">' +
        '<div class="community-three-col">' +
          ...
          '<div class="community-col">' +
            ...
            '<div id="hosts-' + communityId + '" style="display:flex;flex-direction:column;gap:20px">' +
          ...
          '<div class="community-col">' +
            ...
            (events.length
              ? events.map(function(ev) {
                  return '<div class="community-event-item" onclick="SM.showPage(\'events\')" style="cursor:pointer">' +
                    '<div style="font-family:var(--font-head);font-size:1rem;letter-spacing:0.04em;margin-bottom:2px">' + SM._escapeHtml(ev.title) + '</div>' +
                    '<div style="font-size:var(--p3);color:var(--gray-600)">' + ev.date + (ev.time ? ' · ' + ev.time : '') + '</div>' +
                  '</div>';
                }).join('')
...
SM.loadCommunityMembers = function(communityId) {
  ...
  db.collection('users')
    ...
    .then(function(snap) {
      ...
      var total = snap.size;
      var shown = snap.docs.slice(0, 12);
      ...
      shown.forEach(function(doc) {
        ...
        var initials = ((u.firstName||'')[0]||'?').toUpperCase() +
                       ((u.lastInitial||'')[0]||'?').toUpperCase();
        var uid = doc.id;
        html += '<div class="member-card" onclick="SM.viewProfile(\'' + uid + '\')">' +
          (u.avatarURL
            ? '<img src="' + u.avatarURL + '" class="member-avatar"/>' 
            : '<div class="member-avatar member-avatar-initials">' + initials + '</div>'
          ) +
          '<div class="member-name">' + SM._escapeHtml((u.firstName||'')+' '+(u.lastInitial||'')) + '</div>' +
      ...
      if (total > 12) {
        html += '<p style="font-size:var(--p3);color:var(--gray-600);margin-top:10px">+ ' + (total - 12) + ' more members</p>';
      }
...
SM._renderHostCards = function(container, communityId, docs) {
  ...
  if (docs && docs.length > 0) {
    ...
    docs.forEach(function(doc) {
      ...
      /* Avatar — real photo or initials placeholder */
      var avatarDiv = document.createElement('div');
      if (u.avatarURL) {
        avatarDiv.style.cssText = 'width:56px;height:56px;border-radius:50%;overflow:hidden;flex-shrink:0;';
        ...
        avatarDiv.className = 'host-img-placeholder';
        avatarDiv.style.cssText = 'width:56px;height:56px;border-radius:50%;background:var(--teal);display:flex;align-items:center;justify-content:center;flex-shrink:0;';
        avatarDiv.style.fontFamily = 'var(--font-head)';
        avatarDiv.style.fontSize = '1.2rem';
        avatarDiv.style.color = 'var(--white)';
      ...
      roleTag.style.cssText = 'font-family:var(--font-head);font-size:0.6rem;letter-spacing:0.1em;padding:2px 6px;border-radius:2px;margin-left:6px;vertical-align:middle;background:var(--red);color:var(--white);';
      roleTag.textContent = 'HOST';
      nameDiv.appendChild(roleTag);

      var bioDiv = document.createElement('div');
      bioDiv.className = 'host-bio';
      ...
      if (u.website) {
        ...
        link.href = 'https://' + u.website.replace(/^https?:\/\//,'');
        link.target = '_blank';
        link.textContent = 'VIEW PORTFOLIO →';
        infoDiv.appendChild(link);
      } else {
        var profileLink = document.createElement('a');
        profileLink.className = 'host-link';
  ...
  db.collection('communities').doc(communityId)
    ...
    .then(function(snap) {
      ...
      snap.forEach(function(doc) {
        ...
        /* Quote block if this is a reply-to-reply */
        if (r.replyTo) {
          var quoteDiv = document.createElement('div');
          quoteDiv.className = 'chat-reply-quote';
          quoteDiv.style.marginBottom = '6px';
          quoteDiv.innerHTML =
