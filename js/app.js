...
SM._eventsFilter = null; /* null = show all, string = filter by communityId */
...
SM.renderEventsList = function() {
  ...
  SM.loadEvents().then(function(events) {
    SM.loadRsvpState().then(function() {
      /* Apply community filter if set — e.g. navigating from community page */
      var filtered = SM._eventsFilter
        ? events.filter(function(ev) { return ev.community === SM._eventsFilter; })
        : events;
      SM._eventsFilter = null; /* Clear after use */

      container.innerHTML = filtered.length
        ? filtered.map(SM.renderEventCard).join('')
        : '<p class="p2" style="color:var(--gray-600)">No events yet. Check back soon.</p>';
      filtered.forEach(function(ev) { SM.loadComments(ev.id); SM._refreshRsvpCounts(ev.id, ev.community); });
      setTimeout(SM.initDescToggles, 50);
    });
  });
};
...
SM._renderNotifList = function(items) {
  ...
  list.innerHTML = items.slice(0, 5).map(function(n) {
    return SM._buildNotifItemHTML(n);
  }).join('');

  var seeAllEl = document.getElementById('notif-see-all');
  if (seeAllEl) {
    if (items.length > 5) {
      seeAllEl.style.display = 'block';
      seeAllEl.textContent = 'SEE ALL ' + items.length + ' NOTIFICATIONS';
    } else {
...
SM.openNotifLightbox = function() {
  ...
  if (listEl) {
    listEl.innerHTML = SM._allNotifItems.length
