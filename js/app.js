...
SM._renderNotifList = function(items) {
  ...
  list.innerHTML = items.map(function(n) {
    ...
    return '<div class="notif-item' + (n.read ? '' : ' unread') + '" ' +
      'onclick="SM._handleNotifClick(\'' + n.id + '\',\'' + (n.linkPage||'') + '\',\'' + (n.linkEventId||'') + '\',\'' + (n.linkCommunityId||'') + '\')">' +
...
SM._handleNotifClick = function(notifId, page, eventId, communityId) {
  ...
  if (page === 'chat') {
    /* Route to main chat page, switch to the right room */
    SM.showPage('chat');
    if (communityId) {
      setTimeout(function() { SM.renderChatRoom(communityId); }, 100);
    }
  } else if (page === 'smdc' || page === 'smwa' || page === 'smmd') {
    /* Community page — open chat tab */
    SM.showPage(page);
    setTimeout(function() {
    ...
    /* Community board — open board tab */
    if (communityId) {
...
══════════════════════════════════════════════════════════ */
SM.notify = function(recipientUserId, type, message, linkPage, linkEventId, linkCommunityId) {
