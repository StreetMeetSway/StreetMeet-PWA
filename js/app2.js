...
SM.renderCommunityPage = function(communityId) {
  ...
  db.collection('communities').doc(communityId).get().then(function(snap) {
    ...
    if (fsData.headerImageURL && fsData.headerImageURL !== data.headerImageURL) {
      data.headerImageURL = fsData.headerImageURL;
      /* Re-render hero section only */
      var hero = el.querySelector('.community-hero');
      if (hero) {
        hero.className = 'community-hero community-hero--has-image';
        hero.innerHTML =
          '<div class="community-hero-bg" style="background-image:url(' + JSON.stringify(fsData.headerImageURL) + ')"></div>' +
          '<img class="community-hero-logo" src="' + fsData.headerImageURL + '" alt="' + SM._escapeHtml(data.fullName) + '"/>' +
...
SM._renderCommunityPageContent = function(communityId, el, data) {
  ...
  el.innerHTML =
    /* Task 12: Header image — shows logo/photo when headerImageURL set on community */
    '<div class="community-hero' + (data.headerImageURL ? ' community-hero--has-image' : '') + '">' +
      ...
      (data.headerImageURL
        ? '<img class="community-hero-logo" src="' + data.headerImageURL + '" alt="' + SM._escapeHtml(data.fullName) + '"/>'
