...
/* ── Canvas → PNG Blob → Web Share / Download fallback ── */
  ...
  return new Promise(function(resolve, reject) {
    canvas.toBlob(function(blob) {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({ files: [file], title: title, text: text, url: url })
          .then(resolve)
          .catch(function(e) {
            if (e.name === 'AbortError') { resolve(); return; }
            /* Fall back to URL share if file share fails */
            SM._share(title, text, url);
            resolve();
          });
      } else if (navigator.share) {
        /* URL-only share (desktop Safari, Firefox) */
        SM._share(title, text, url);
        resolve();
      } else {
        ...
        SM.showToast('Card downloaded — share from your photos!', 'success');
        resolve();
      }
    }, 'image/png');
  });
