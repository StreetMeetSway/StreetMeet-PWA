  ...
  db.collection('communities').doc(communityId)
    ...
    .then(function(doc) {
      ...
      shareBtn.addEventListener('click', function() {
        SM.shareThread(communityId, threadId, t.title);
      });
...
/* ══════════════════════════════════════════════════════════
   TASK 08 & 09: SOCIAL SHARING
   Web Share API with clipboard fallback
══════════════════════════════════════════════════════════ */

SM._share = function(title, text, url) {
  if (navigator.share) {
    navigator.share({ title: title, text: text, url: url })
      .catch(function(e) {
        /* User cancelled or share failed — silent */
        if (e.name !== 'AbortError') console.warn('SM: share error', e);
      });
  } else {
    /* Clipboard fallback for desktop browsers */
    if (navigator.clipboard) {
      navigator.clipboard.writeText(url)
        .then(function() { SM.showToast('Link copied to clipboard!', 'success'); })
        .catch(function() { SM.showToast('Copy this link: ' + url, 'success'); });
    } else {
...
/* Build the base URL for deep links — updated to streetmeet.com after Task 13 */
SM._baseURL = function() {
  return window.location.origin + window.location.pathname.replace(/\/+$/, '');
};

/* Task 08: Share an event */
SM.shareEvent = function(evId) {
  ...
  SM._share(ev.title || 'StreetMeet Event', text, url);
};

/* Task 09: Share a board thread */
SM.shareThread = function(communityId, threadId, title) {
  var url  = SM._baseURL() + '#board/' + communityId + '/' + threadId;
  var text = (SM.communityData[communityId] ? SM.communityData[communityId].fullName : communityId.toUpperCase()) +
