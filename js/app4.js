...
SM.renderCommunityPage = function(communityId) {
  ...
  db.collection('communities').doc(communityId).get().then(function(snap) {
    if (snap.exists && snap.data().headerImageURL) {
      data.headerImageURL = snap.data().headerImageURL;
    }
    /* Now render with complete data — load events then re-render for fresh counts */
    SM._renderCommunityPageContent(communityId, el, data);
    SM.loadEvents().then(function() {
      SM._renderCommunityPageContent(communityId, el, data);
    });
  }).catch(function() {
    /* Firestore unavailable — render with cached data */
    SM._renderCommunityPageContent(communityId, el, data);
    SM.loadEvents().then(function() {
      SM._renderCommunityPageContent(communityId, el, data);
    });
