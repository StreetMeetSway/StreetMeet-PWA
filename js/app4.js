...
/* ── Canvas → PNG Blob → share modal ── */
/* Web Share API with files requires a synchronous user gesture.
   Since canvas generation is async we show a lightweight share
   modal with a Download button (saves to Camera Roll on mobile)
   and a Copy Link button — user can then share from Photos. */
SM._shareCanvasCard = function(canvas, filename, title, text, url) {
  return new Promise(function(resolve) {
    canvas.toBlob(function(blob) {
      ...
      /* Try URL-only native share first (works within gesture on some browsers) */
      if (navigator.share) {
        navigator.share({ title: title, text: text, url: url })
          .then(function() {
            /* Also offer image download after share */
            SM._offerImageDownload(objectURL, filename);
            resolve();
          })
          .catch(function(e) {
        ...
        /* No native share — show download option */
        SM._offerImageDownload(objectURL, filename);
        resolve();
      }
    }, 'image/png');
...
/* Offer image download via a toast-style prompt */
SM._offerImageDownload = function(objectURL, filename) {
  /* Remove any existing share prompt */
  var existing = document.getElementById('sm-share-prompt');
  if (existing) existing.remove();

  var prompt = document.createElement('div');
  prompt.id = 'sm-share-prompt';
  prompt.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);' +
    'background:var(--black);color:var(--white);padding:16px 20px;z-index:9999;' +
    'font-family:var(--font-b);font-size:0.85rem;display:flex;align-items:center;' +
    'gap:12px;box-shadow:0 8px 32px rgba(0,0,0,0.4);max-width:360px;width:90%;';

  prompt.innerHTML =
    '<span style="flex:1;line-height:1.4">Save image then share to Instagram, iMessage &amp; more</span>' +
    '<a id="sm-share-dl" href="' + objectURL + '" download="' + filename + '" ' +
      'style="background:var(--teal);color:var(--white);padding:8px 14px;' +
      'font-family:var(--font-head);font-size:0.85rem;letter-spacing:0.06em;' +
      'text-decoration:none;white-space:nowrap;flex-shrink:0;">SAVE IMAGE</a>' +
    '<button id="sm-share-close-btn" ' +
      'style="background:none;border:none;color:rgba(255,255,255,0.5);cursor:pointer;' +
      'font-size:1.2rem;padding:0;flex-shrink:0;line-height:1;">✕</button>';

  document.body.appendChild(prompt);

  /* Wire close button via addEventListener — avoids inline quote conflicts */
  var closeBtn = document.getElementById('sm-share-close-btn');
  if (closeBtn) {
  ...
  /* Clean up object URL after download click */
  document.getElementById('sm-share-dl').addEventListener('click', function() {
    setTimeout(function() {
      URL.revokeObjectURL(objectURL);
      prompt.remove();
    }, 2000);
  ...
  setTimeout(function() {
    if (document.getElementById('sm-share-prompt')) {
      prompt.remove();
      URL.revokeObjectURL(objectURL);
    }
  }, 12000);
