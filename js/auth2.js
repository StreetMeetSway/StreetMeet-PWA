...
/* ── User self-deletion ── */
SM.deleteOwnAccount = function() {
  var user = SM.getCurrentUser();
  if (!user) return;

  var confirmed = confirm(
    'Are you sure you want to permanently delete your account?\n\n' +
    '• Your profile and all uploaded photos will be removed\n' +
    '• Your posts and chat messages will remain but show as deleted user\n' +
  ...
  if (!typed || typed.trim().toUpperCase() !== 'DELETE') {
    SM.showToast('Account deletion cancelled', 'success');
    return;
  }

  var fbUser = firebase.auth().currentUser;
  if (!fbUser) { SM.showToast('Please sign in again to delete your account', 'error'); return; }

  SM.showToast('Deleting account...', 'success');

  /* Step 1: Delete Storage files — avatar and portfolio photos */
  var storageDeletePromises = [];

  if (user.avatarURL && user.avatarURL.includes('firebasestorage')) {
    try {
      storageDeletePromises.push(
        firebase.storage().refFromURL(user.avatarURL).delete().catch(function(){})
      );
    } catch(e) {}
  }
  ...
  photos.forEach(function(url) {
    if (url && url.includes('firebasestorage')) {
      try {
        storageDeletePromises.push(
          firebase.storage().refFromURL(url).delete().catch(function(){})
        );
      } catch(e) {}
    }
  });

  Promise.all(storageDeletePromises).then(function() {
    ...
    /* Step 4: Clear local state and redirect to landing */
    try {
      localStorage.removeItem('sm_current_user');
      localStorage.removeItem('sm_last_page');
      localStorage.removeItem('sm_last_profile_uid');
      sessionStorage.clear();
    } catch(e) {}
    SM.showToast('Your account has been deleted', 'success');
    setTimeout(function() { window.location.reload(); }, 1500);
  }).catch(function(err) {
    if (err.code === 'auth/requires-recent-login') {
      SM.showToast('Please sign out and sign back in, then try again', 'error');
    } else {
      SM.showToast('Could not delete account — please try again', 'error');
      console.error('SM: deleteOwnAccount error:', err);
    }
  });
};
