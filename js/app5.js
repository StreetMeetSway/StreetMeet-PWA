...
SM.loadEvents = function() {
  ...
  return Promise.all(promises)
    .then(function(results) {
      ...
      /* Opt 1: Read RSVP counts from event doc fields (no subcollection reads) */
      SM.events.forEach(function(ev) {
        ev._goingCount    = ev.goingCount    || 0;
        ev._maybeCount    = ev.maybeCount    || 0;
        ev._notgoingCount = ev.notgoingCount || 0;
      });
      return SM.events;
    })
    .catch(function(err) {
...
SM.openEventLightbox = function(evId) {
  ...
  content.innerHTML =
    ...
    '<div style="padding:24px 28px;">' +
      ...
      /* Share button */
      '<button onclick="SM.shareEvent(\'' + evId + '\')" ' +
        'style="display:inline-flex;align-items:center;gap:6px;margin-bottom:16px;' +
        'background:none;border:1px solid var(--gray-200);padding:6px 14px;cursor:pointer;' +
        'font-family:var(--font-m);font-size:0.65rem;letter-spacing:0.1em;color:var(--gray-600);' +
...
SM.renderEventCard = function(ev) {
  ...
  return '<div class="event-card" id="event-card-' + ev.id + '">' +
    ...
    '<div class="event-card-img" style="cursor:pointer" onclick="SM.openEventLightbox(\'' + ev.id + '\')">' +
      (ev.photo
        ? '<img src="' + ev.photo + '" alt="' + SM._escapeHtml(ev.title) + '" style="width:100%;height:100%;object-fit:cover;display:block;"/>'
        : '<div class="event-card-img-placeholder">' +
            '<div style="font-family:var(--font-head);font-size:4rem;color:rgba(255,255,255,0.08);letter-spacing:0.1em">' + ev.community.toUpperCase() + '</div>' +
          '</div>'
      ) +
    ...
    '<div class="event-card-body">' +
      ...
      '<div class="rsvp-counts">' +
        '<div class="rsvp-count-item"><strong id="going-' + ev.id + '">' + (ev._goingCount||0) + '</strong> I\'m Down</div>' +
        '<div class="rsvp-count-item"><strong id="maybe-' + ev.id + '">' + (ev._maybeCount||0) + '</strong> Just Might</div>' +
        '<div class="rsvp-count-item"><strong id="notgoing-' + ev.id + '">' + (ev._notgoingCount||0) + '</strong> Missing Out</div>' +
      ...
      '</div>' +
      /* Task 08: Share button on event card */
      '<button onclick="SM.shareEvent(\'' + ev.id + '\')" ' +
        'style="display:inline-flex;align-items:center;gap:6px;margin-top:12px;' +
        'background:none;border:1px solid var(--gray-200);padding:7px 14px;cursor:pointer;' +
        'font-family:var(--font-m);font-size:0.65rem;letter-spacing:0.1em;color:var(--gray-600);' +
...
SM.setRsvp = function(evId, state) {
  ...
  /* Build event doc increment update */
  var inc = firebase.firestore.FieldValue.increment;
  var eventDocUpdate = {};
  /* Decrement previous state count on event doc */
  if (prev) eventDocUpdate[prev + 'Count'] = inc(-1);
  /* Increment new state count on event doc */
  eventDocUpdate[state + 'Count'] = inc(1);

  var eventRef = db.collection('communities').doc(ev.community)
    .collection('events').doc(evId);

  /* Write both atomically — rsvp doc + event count fields */
  var batch = db.batch();
  batch.set(rsvpRef, {
    userId:    user.id,
    state:     state,
  ...
  batch.update(eventRef, eventDocUpdate);

  batch.commit().then(function() {
    /* Update local cache */
    if (prev) ev['_' + prev + 'Count'] = Math.max(0, (ev['_' + prev + 'Count'] || 1) - 1);
    ev['_' + state + 'Count'] = (ev['_' + state + 'Count'] || 0) + 1;
...
SM.viewProfile = function(userId) {
  ...
  /* If viewing own profile, use cached data and clear any stored uid */
  if (current && current.id === userId) {
    try { localStorage.removeItem('sm_last_profile_uid'); } catch(e) {}
    SM.renderProfile();
    SM.showPage('profile');
    return;
  }
  /* Store the viewed userId so refresh can restore it */
  try { localStorage.setItem('sm_last_profile_uid', userId); } catch(e) {}
...
SM._renderProfileData = function(user, isOwn) {
  ...
  var photosHTML;
  if (isPhotoType) {
    /* Task 04: Photographers & Models always see the 2-col public grid —
       whether viewing own or another user's profile. EDIT PROFILE button
       in the header handles editing. */
    var filledPhotos = photos.filter(function(url) { return !!url; });
    if (filledPhotos.length === 0) {
      photosHTML = '<p class="p2" style="color:var(--gray-600)">No photos yet.' +
        (isOwn ? ' <button class="see-more-btn" onclick="SM.showPage(\'edit-profile\')">Add photos →</button>' : '') +
        '</p>';
    } else {
    ...
    /* Non-photo creator type — 4-col grid with edit slots when own profile */
  ...
  if (isVideoType) {
    ...
    rightColHTML =
      ...
      '<div class="' + (isPhotoType ? 'profile-photos-grid-2col' : 'profile-photos-grid') + '">' + photosHTML + '</div>';
  }
  ...
  if (isPhotoType && SM._pendingLightboxPhotos && SM._pendingLightboxPhotos.length > 0) {
    ...
    '<div class="section" style="max-width:640px;margin:0 auto">' +
      ...
      '<div style="display:flex;flex-direction:column;gap:18px">' +
        ...
        '<div style="margin-top:32px;padding-top:24px;border-top:2px solid var(--gray-200)">' +
          '<div style="font-family:var(--font-m);font-size:0.62rem;letter-spacing:0.15em;text-transform:uppercase;color:var(--gray-600);margin-bottom:12px;">Danger Zone</div>' +
          '<p style="font-size:var(--p3);color:var(--gray-600);margin-bottom:14px;line-height:1.6;">' +
            'Permanently delete your account, profile, and all uploaded photos. This cannot be undone.' +
          '</p>' +
          '<button onclick="SM.deleteOwnAccount()" ' +
            'style="background:none;border:1px solid var(--red);color:var(--red);padding:8px 18px;' +
            'font-family:var(--font-m);font-size:0.68rem;letter-spacing:0.1em;text-transform:uppercase;' +
            'cursor:pointer;transition:background 0.15s;" ' +
            'onmouseover="this.style.background=\'var(--red)\';this.style.color=\'var(--white)\'" ' +
            'onmouseout="this.style.background=\'none\';this.style.color=\'var(--red)\'">' +
            'DELETE MY ACCOUNT' +
          '</button>' +
        '</div>' +
...
SM.renderCommunityPage = function(communityId) {
  ...
  /* Fetch headerImageURL from Firestore first, then render once with full data.
     This avoids the race condition where loadEvents() re-renders and wipes
     the hero image that was patched in by the Firestore callback. */
  /* Render immediately with cached data for instant feedback */
  SM._renderCommunityPageContent(communityId, el, data);

  /* Fetch headerImageURL then update hero directly in DOM — no full re-render needed */
  db.collection('communities').doc(communityId).get().then(function(snap) {
    if (snap.exists && snap.data().headerImageURL) {
      var url = snap.data().headerImageURL;
      data.headerImageURL = url;

      /* Update hero directly — avoids race with loadEvents re-render */
      ...
      if (hero) {
        ...
        /* Hide text elements */
        ['community-code','community-city','community-sub'].forEach(function(cls) {
          var el2 = hero.querySelector('.' + cls);
          if (el2) el2.style.display = 'none';
        });
      }
    }
    /* Re-render only the events column — avoids hero flash */
    SM.loadEvents().then(function() {
      SM._refreshCommunityEvents(communityId, el, data);
    });
  }).catch(function() {
    SM.loadEvents().then(function() {
      SM._refreshCommunityEvents(communityId, el, data);
    ...
    '<div class="community-panel" id="cpanel-about-' + communityId + '">' +
      ...
      '<div class="section" style="padding-bottom:0">' +
        '<div class="two-col" style="align-items:flex-start;gap:48px">' +
          ...
          '<div class="community-meets-col">' +
            '<span class="section-label">Upcoming Meets</span>' +
            (events.length
              ? events.slice(0, 2).map(function(ev) {
                  return '<div class="community-event-item community-event-item--large" onclick="SM.openEventLightbox(\'' + ev.id + '\')" style="margin-bottom:12px;">' +
                    '<div class="community-event-cover">' +
                      (ev.photo
                        ? '<img src="' + ev.photo + '" alt="' + SM._escapeHtml(ev.title) + '"/>'
                        : '<div class="community-event-cover-placeholder"><span>' + (ev.community||'').toUpperCase() + '</span></div>'
                      ) +
                    '</div>' +
                    '<div class="community-event-info--large">' +
                      '<div class="community-event-title--large">' + SM._escapeHtml(ev.title) + '</div>' +
                      '<div class="community-event-meta" style="font-size:var(--p3);color:var(--gray-600);margin-top:2px">' + SM._formatEventDate(ev.date, ev.time) + '</div>' +
                    '</div>' +
                  '</div>';
                }).join('') +
              (events.length > 3
...
SM.loadCommunityHosts = function(communityId) {
  ...
  /* Opt 2: sessionStorage cache — 1 hour TTL */
  var cacheKey = 'sm_hosts_' + communityId;
  try {
  ...
  db.collection('users')
    ...
    .get()
    .then(function(snap) {
      /* Task 06: Filter out test accounts */
      var liveDocs = snap.docs.filter(function(d) { return !d.data().isTestAccount; });
      if (liveDocs.length === 0) {
        SM._renderHostCards(container, communityId, null);
        return;
      }
      SM._renderHostCards(container, communityId, liveDocs);
      /* Opt 2: Cache rendered HTML after _renderHostCards writes it */
      setTimeout(function() {
        try { sessionStorage.setItem(cacheKey, JSON.stringify({html: container.innerHTML, ts: Date.now()})); } catch(e) {}
      }, 100);
    })
    .catch(function(err) {
...
SM.loadCommunityMembers = function(communityId) {
  ...
  /* Opt 2: sessionStorage cache — 1 hour TTL */
  var cacheKey = 'sm_members_' + communityId;
  try {
    var cached = JSON.parse(sessionStorage.getItem(cacheKey) || 'null');
    if (cached && (Date.now() - cached.ts) < 3600000) {
      container.innerHTML = cached.html;
  ...
  db.collection('users')
    ...
    .then(function(snap) {
      ...
      /* Task 06: Filter out test accounts before rendering */
      var liveDocs = snap.docs.filter(function(d) { return !d.data().isTestAccount; });
      var total = liveDocs.length;
      if (total === 0) {
        container.innerHTML = '<p class="p2" style="color:var(--gray-600);font-size:0.85rem">No members yet.</p>';
        return;
      }
      html = '<div style="font-family:var(--font-head);font-size:0.85rem;letter-spacing:0.08em;color:var(--gray-600);margin-bottom:14px;">' +
        total + ' MEMBER' + (total !== 1 ? 'S' : '') + '</div>';
      html += '<div class="members-grid">';
      liveDocs.forEach(function(doc, i) {
        var u = doc.data();
        var initials = ((u.firstName||'')[0]||'?').toUpperCase() +
                       ((u.lastInitial||'')[0]||'?').toUpperCase();
        var uid = doc.id;
      ...
      /* View More button — only shown when > 12 live members */
      if (total > 12) {
      ...
      /* Opt 2: Write to sessionStorage cache */
      try { sessionStorage.setItem(cacheKey, JSON.stringify({html: html, ts: Date.now()})); } catch(e) {}
    })
    .catch(function(err) {
...
SM._renderHostCards = function(container, communityId, docs) {
  ...
  if (docs && docs.length > 0) {
    ...
    docs.forEach(function(doc) {
      ...
      /* Build card as HTML string — onclick attrs survive sessionStorage cache */
      var uid = doc.id;
      var avatarHTML = u.avatarURL
        ? '<img src="' + u.avatarURL + '" alt="' + SM._escapeHtml(u.firstName||'') + '"/>'
        : initials;
      var igHTML = u.instagram
        ? '<div style="font-size:var(--p3);color:var(--teal);margin-bottom:4px;">' +
            '<a href="https://instagram.com/' + u.instagram + '" target="_blank" ' +
               'onclick="event.stopPropagation()" style="color:inherit;">@' + u.instagram + '</a>' +
          '</div>'
        : '';
      var linkHTML = u.website
        ? '<a class="host-link" href="https://' + u.website.replace(/^https?:\/\//, '') + '" ' +
            'target="_blank" onclick="event.stopPropagation()" ' +
            'style="font-size:0.72rem;margin-top:6px;display:inline-block;">VIEW PORTFOLIO →</a>'
        : '<a class="host-link" onclick="event.stopPropagation();SM.viewProfile(\'' + uid + '\')" ' +
            'style="font-size:0.72rem;margin-top:6px;display:inline-block;cursor:pointer;">VIEW PROFILE →</a>';

      var card = document.createElement('div');
      card.className = 'host-card-col';
      card.style.cursor = 'pointer';
      card.setAttribute('onclick', "SM.viewProfile('" + uid + "')");
      card.innerHTML =
        '<div class="host-avatar-col">' + avatarHTML + '</div>' +
        '<div class="host-info-col">' +
          '<div class="host-name-col">' +
            SM._escapeHtml(((u.firstName||'') + ' ' + (u.lastInitial||'')).trim().toUpperCase()) +
            '<span style="font-family:var(--font-head);font-size:0.58rem;letter-spacing:0.1em;' +
              'padding:2px 5px;border-radius:2px;background:var(--red);color:var(--white);flex-shrink:0;">' +
              'HOST</span>' +
          '</div>' +
          igHTML +
          '<div class="host-bio-col">' + SM._escapeHtml(u.bio||'') + '</div>' +
          linkHTML +
        '</div>';
...
SM.openThread = function(communityId, threadId) {
  ...
  db.collection('communities').doc(communityId)
    ...
    .then(function(doc) {
      ...
      /* Action buttons row — share (all users) + edit (author only) */
      var actionRow = document.createElement('div');
      actionRow.style.cssText = 'display:flex;align-items:center;gap:8px;flex-shrink:0;';

      /* Task 09: Share button — visible to all users */
      var shareBtn = document.createElement('button');
      shareBtn.style.cssText = 'display:inline-flex;align-items:center;gap:5px;background:none;' +
        'border:1px solid var(--gray-200);padding:3px 10px;cursor:pointer;' +
        'font-family:var(--font-m);font-size:0.65rem;letter-spacing:0.08em;color:var(--gray-600);' +
        'transition:border-color 0.15s,color 0.15s;';
      shareBtn.innerHTML = '<svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2">' +
        '<circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/>' +
        '<line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/>' +
        '<line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>SHARE';
      shareBtn.addEventListener('mouseover', function() {
        this.style.borderColor = 'var(--teal)'; this.style.color = 'var(--teal)';
      });
      shareBtn.addEventListener('mouseout', function() {
        this.style.borderColor = 'var(--gray-200)'; this.style.color = 'var(--gray-600)';
      });
      shareBtn.addEventListener('click', function() {
        SM.shareThread(communityId, threadId, t.title, t.body, t.imageURL || null);
      });
...
/* Task 15: Password strength checker */
SM._checkPasswordStrength = function(pw) {
  ...
  var levels = [
    { pct: '20%', color: '#c00000', text: 'TOO WEAK' },
    { pct: '40%', color: '#c00000', text: 'WEAK' },
    { pct: '60%', color: '#D4850A', text: 'FAIR' },
    { pct: '80%', color: '#4a9b96', text: 'GOOD' },
    { pct: '100%', color: '#2e7d4f', text: 'STRONG' }
  ];
...
/* Task 05: Resend verification email */
SM.resendVerificationEmail = function() {
  var fbUser = firebase.auth().currentUser;
  if (!fbUser) return;
  fbUser.sendEmailVerification()
    .then(function() { SM.showToast('Verification email sent — check your inbox', 'success'); })
    .catch(function() { SM.showToast('Could not send email — try again shortly', 'error'); });
};



/* ══════════════════════════════════════════════════════════
   IMAGE CARD GENERATOR — Tasks 08 & 09
   Generates a 1080×1080 share card via canvas
   then shares as a PNG file via Web Share API
══════════════════════════════════════════════════════════ */

/* Load a cross-origin image onto canvas — returns Promise<HTMLImageElement> */
SM._loadCORSImage = function(url) {
  return new Promise(function(resolve, reject) {
    var img = new Image();
...
/* Wrap text to fit a given canvas width — returns array of lines */
SM._wrapCanvasText = function(ctx, text, maxWidth) {
...
SM._roundRect = function(ctx, x, y, w, h, r) {
  ctx.beginPath();
  ctx.moveTo(x + r, y);
  ctx.lineTo(x + w - r, y);
...
/* ── Generate event share card ── */
SM._generateEventCard = function(ev) {
  var SIZE = 1080;
  var canvas = document.createElement('canvas');
  canvas.width  = SIZE;
  canvas.height = SIZE;
  ...
  function drawCard(photoImg) {
    ...
    if (photoImg) {
      /* Top half — event photo */
      var photoH = Math.round(SIZE * 0.55);
      ctx.save();
      ctx.rect(0, 0, SIZE, photoH);
      ctx.clip();
      /* Center-crop the photo */
      var scale = Math.max(SIZE / photoImg.naturalWidth, photoH / photoImg.naturalHeight);
      var sw = SIZE / scale;
      var sh = photoH / scale;
      var sx = (photoImg.naturalWidth  - sw) / 2;
      var sy = (photoImg.naturalHeight - sh) / 2;
      ctx.drawImage(photoImg, sx, sy, sw, sh, 0, 0, SIZE, photoH);
      ctx.restore();

      /* Gradient overlay on photo bottom */
      var grad = ctx.createLinearGradient(0, photoH * 0.5, 0, photoH);
    ...
    var textTop = photoImg ? Math.round(SIZE * 0.56) : 160;

    /* Community label pill */
    var label = (ev.communityLabel || ev.community || 'STREETMEET').toUpperCase();
    ctx.font = '500 32px "DM Mono", monospace';
    ctx.fillStyle = '#4a9b96';
    ctx.letterSpacing = '6px';
    ctx.fillText(label, 64, textTop + 48);

    /* Event title */
    ctx.font = '900 ' + (ev.title && ev.title.length > 20 ? '88' : '108') + 'px "Bebas Neue", sans-serif';
    ctx.fillStyle = '#ffffff';
    ctx.letterSpacing = '2px';
    var titleLines = SM._wrapCanvasText(ctx, (ev.title || '').toUpperCase(), SIZE - 128);
    var titleY = textTop + 110;
    titleLines.slice(0, 3).forEach(function(line) {
      ctx.fillText(line, 64, titleY);
      titleY += parseInt(ctx.font) * 1.1;
    });
    ...
    if (ev.date) {
      ...
      if (ev.time) {
        ...
        var h = parseInt(tp[0]);
        var m = tp[1];
        dateStr += '  ·  ' + (h > 12 ? h - 12 : h || 12) + (m !== '00' ? ':' + m : '') + (h >= 12 ? 'PM' : 'AM');
      }
    }
    ...
    ctx.fillText('STREETMEET', 64, barY + 52);

    /* Domain */
    ctx.font = '400 26px "DM Mono", monospace';
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.letterSpacing = '1px';
    ctx.fillText('streetmeet.com', SIZE - 64 - ctx.measureText('streetmeet.com').width, barY + 50);
  }

  if (ev.photo) {
    return SM._loadCORSImage(ev.photo)
      .then(function(img) { drawCard(img); return canvas; })
      .catch(function()   { drawCard(null); return canvas; });
  } else {
    drawCard(null);
...
/* ── Generate board thread share card ── */
SM._generateThreadCard = function(communityId, threadId, title, body, imageURL) {
  var SIZE = 1080;
  var canvas = document.createElement('canvas');
  canvas.width  = SIZE;
  canvas.height = SIZE;
  ...
  function drawCard(photoImg) {
    ...
    if (photoImg) {
      var photoH = Math.round(SIZE * 0.45);
      ctx.save();
      ctx.rect(0, 0, SIZE, photoH);
      ctx.clip();
      var scale = Math.max(SIZE / photoImg.naturalWidth, photoH / photoImg.naturalHeight);
      var sw = SIZE / scale, sh = photoH / scale;
      var sx = (photoImg.naturalWidth - sw) / 2, sy = (photoImg.naturalHeight - sh) / 2;
      ctx.drawImage(photoImg, sx, sy, sw, sh, 0, 0, SIZE, photoH);
      ctx.restore();
      var grad = ctx.createLinearGradient(0, photoH * 0.4, 0, photoH);
      grad.addColorStop(0, 'rgba(0,0,0,0)');
    ...
    var textTop = photoImg ? Math.round(SIZE * 0.46) : 140;

    /* Community label */
    var commData = SM.communityData[communityId];
    var label = commData ? commData.fullName.toUpperCase() : communityId.toUpperCase();
    ctx.font = '500 30px "DM Mono", monospace';
    ctx.fillStyle = '#4a9b96';
    ctx.letterSpacing = '5px';
    ctx.fillText(label + '  ·  DISCUSSION BOARD', 64, textTop + 44);

    /* Thread title */
    ctx.font = '900 ' + (title && title.length > 25 ? '76' : '96') + 'px "Bebas Neue", sans-serif';
    ctx.fillStyle = '#ffffff';
    ctx.letterSpacing = '1px';
    var titleLines = SM._wrapCanvasText(ctx, (title || '').toUpperCase(), SIZE - 128);
    var titleY = textTop + 116;
    titleLines.slice(0, 3).forEach(function(line) {
      ctx.fillText(line, 64, titleY);
      titleY += parseInt(ctx.font) * 1.1;
    });
    ...
    /* Body excerpt */
    if (body) {
      var excerpt = body.length > 120 ? body.substring(0, 117) + '...' : body;
      ctx.font = '300 34px "DM Sans", sans-serif';
      ctx.fillStyle = 'rgba(255,255,255,0.65)';
      ctx.letterSpacing = '0px';
      var bodyLines = SM._wrapCanvasText(ctx, excerpt, SIZE - 128);
      var bodyY = titleY + 24;
      bodyLines.slice(0, 3).forEach(function(line) {
        ctx.fillText(line, 64, bodyY);
        bodyY += 48;
      });
    }

    /* Bottom bar */
    var barH = 80, barY = SIZE - barH;
    ctx.fillStyle = 'rgba(0,0,0,0.6)';
    ctx.fillRect(0, barY, SIZE, barH);
    ctx.fillStyle = '#4a9b96';
    ctx.fillRect(0, barY, SIZE, 3);
  ...
  if (imageURL) {
    return SM._loadCORSImage(imageURL)
      .then(function(img) { drawCard(img); return canvas; })
      .catch(function()   { drawCard(null); return canvas; });
  } else {
    drawCard(null);
...
/* ── Canvas → Share Preview Modal ──
   Shows generated card as a visible image in a fullscreen modal.
   User long-presses (iOS) or clicks SAVE IMAGE (Android/Desktop)
   to save to Camera Roll, then shares from Photos.
   SHARE LINK copies/shares the deep link URL separately.
   No async gesture issues — modal opens synchronously,
   image renders inside it after canvas generation completes. ── */

SM._shareCanvasCard = function(canvas, filename, title, text, url) {
  return new Promise(function(resolve) {
    canvas.toBlob(function(blob) {
      if (!blob) {
        /* Blob failed — fall back to URL share */
        if (navigator.share) {
          navigator.share({ title: title, text: text, url: url }).catch(function(){});
        } else if (navigator.clipboard) {
          navigator.clipboard.writeText(url).then(function() {
            SM.showToast('Link copied!', 'success');
          });
        }
        resolve(); return;
      }
      ...
      SM._showShareModal(objectURL, filename, title, text, url);
      resolve();
    }, 'image/png');
  });
};
...
/* ── Share Preview Modal ── */
SM._showShareModal = function(objectURL, filename, title, text, url) {
  /* Remove any existing modal */
  var existing = document.getElementById('sm-share-modal');
  if (existing) { existing.remove(); }

  /* Detect iOS for instruction copy */
  var isIOS = /iphone|ipad|ipod/i.test(navigator.userAgent);
  var isAndroid = /android/i.test(navigator.userAgent);
  var isMobile = isIOS || isAndroid;

  /* Instruction text per platform */
  var instruction = isIOS
    ? 'Hold image to save → share from Photos'
    : isAndroid
      ? 'Tap SAVE IMAGE → share from your gallery'
      : 'Right-click image to save → share anywhere';

  /* Build modal */
  var overlay = document.createElement('div');
  overlay.id = 'sm-share-modal';
  ...
  label.style.cssText = 'color:rgba(255,255,255,0.6);font-family:var(--font-m);' +
    'font-size:0.72rem;letter-spacing:0.1em;text-transform:uppercase;text-align:center;margin:0;';
  ...
  /* SAVE IMAGE button — download anchor */
  ...
  saveBtn.style.cssText = [
    ...
    'padding:12px 24px;',
    'font-family:var(--font-head);font-size:1rem;letter-spacing:0.1em;',
  ...
  linkBtn.addEventListener('click', function() {
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(url).then(function() {
        SM.showToast('Link copied to clipboard!', 'success');
      });
    } else {
      SM.showToast('Copy this link: ' + url, 'success');
    }
  });
  ...
  closeBtn.style.cssText = [
    'background:none;color:rgba(255,255,255,0.4);border:none;',
    'font-family:var(--font-m);font-size:0.72rem;letter-spacing:0.1em;',
    'cursor:pointer;margin-top:4px;padding:8px;'
  ].join('');
  ...
  /* Close on overlay tap (outside image) */
...
/* ── Public share entry points ── */

/* Task 08: Share event as image card */
/* Build the base URL for deep links — auto-updates to streetmeet.com after Task 13 */
SM._baseURL = function() {
  return window.location.origin + window.location.pathname.replace(/\/+$/, '');
};

SM.shareEvent = function(evId) {
  var ev = SM.events.find(function(e) { return e.id === evId; });
  if (!ev) return;
  var url  = SM._baseURL() + '#event/' + evId;
  var text = (ev.communityLabel || ev.community.toUpperCase()) +
             ' — ' + (ev.title || 'Street Meet') +
             ' · ' + SM._formatEventDate(ev.date, ev.time);
  var title = ev.title || 'StreetMeet Event';

  SM.showToast('Generating share card...', 'success');
  SM._generateEventCard(ev)
    .then(function(canvas) {
      return SM._shareCanvasCard(canvas, 'streetmeet-event.png', title, text, url);
    })
    .catch(function(err) {
      console.error('SM: shareEvent error:', err);
      SM._share(title, text, url);
    });
};

/* Task 09: Share board thread as image card */
SM.shareThread = function(communityId, threadId, title, body, imageURL) {
  var url  = SM._baseURL() + '#board/' + communityId + '/' + threadId;
  var commData = SM.communityData[communityId];
  var text = (commData ? commData.fullName : communityId.toUpperCase()) +
             ' Board — ' + (title || 'Discussion');

  SM.showToast('Generating share card...', 'success');
  SM._generateThreadCard(communityId, threadId, title, body, imageURL)
    .then(function(canvas) {
      return SM._shareCanvasCard(canvas, 'streetmeet-thread.png', title || 'StreetMeet Discussion', text, url);
    })
    .catch(function(err) {
      console.error('SM: shareThread error:', err);
      SM._share(title || 'StreetMeet Discussion', text, url);
    });
};


/* ── Deep link router — runs on app init ── */
SM._handleDeepLink = function() {
  var hash = window.location.hash;
  if (!hash) return;

  /* #event/{evId} */
  var evMatch = hash.match(/^#event\/([\w-]+)$/);
  if (evMatch) {
    var evId = evMatch[1];
    SM.loadEvents().then(function() {
      var ev = SM.events.find(function(e) { return e.id === evId; });
      if (ev) {
        SM.showPage('events');
        setTimeout(function() { SM.openEventLightbox(evId); }, 300);
      }
    });
    return;
  }

  /* #board/{communityId}/{threadId} */
  var boardMatch = hash.match(/^#board\/([\w-]+)\/([\w-]+)$/);
  if (boardMatch) {
    var communityId = boardMatch[1];
    var threadId    = boardMatch[2];
    SM.showPage(communityId);
    setTimeout(function() {
      SM.switchCommunityTab(communityId, 'board', null);
      setTimeout(function() { SM.openThread(communityId, threadId); }, 400);
    }, 300);
    return;
  }
};
...
SM.updateNav = function(activePageId) {
  ...
  /* Task 05: Show email verification banner if user signed in but unverified */
  var fbUser = firebase.auth().currentUser;
  var verifyBanner = document.getElementById('verify-email-banner');
  if (verifyBanner) {
    verifyBanner.style.display = (fbUser && !fbUser.emailVerified && user) ? 'flex' : 'none';
  }
  var navLinksEl = document.getElementById('nav-links');
  var navUserEl  = document.getElementById('nav-user');
  if (!navLinksEl) return;
...
/* ── Refresh only the Upcoming Meets section — no hero flash ── */
SM._refreshCommunityEvents = function(communityId, el, data) {
  var todayStr = new Date().toISOString().split('T')[0];
  var events = SM.events.filter(function(e) {
    return e.community === communityId && e.date >= todayStr;
  });

  /* Find the meets container inside the about panel */
  var panel = el.querySelector('#cpanel-about-' + communityId);
  if (!panel) return;

  var meetsCol = panel.querySelector('.community-meets-col');
  if (!meetsCol) return;

  /* Re-render just the meets column content */
      ...
      users.map(function(u) {
        return '<tr>' +
          ...
          '<td><div class="action-btns">' +
            ...
            (u.role === 'host' ? '<button class="action-btn promote" style="background:var(--black);color:var(--white);border-color:var(--black);" onclick="SM.promoteToAdmin(\'' + u.id + '\',\'' + SM._escapeHtml((u.firstName||'') + ' ' + (u.lastInitial||'')).trim() + '\')">MAKE ADMIN</button>' : '') +
...
SM.init = function() {
  ...
  if (user) {
    try {
      ...
      var lastProfileUid = localStorage.getItem('sm_last_profile_uid');
  ...
  /* If last page was another user's profile, restore it via viewProfile */
  if (startPage === 'profile' && typeof lastProfileUid !== 'undefined' && lastProfileUid) {
    SM.showPage('profile');
    SM.viewProfile(lastProfileUid);
  } else {
    SM.showPage(startPage);
  }

  /* Tasks 08/09: Handle deep links — #event/{id} or #board/{community}/{threadId} */
  SM._handleDeepLink();
};
