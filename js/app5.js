...
SM.loadEvents = function() {
  ...
  return Promise.all(promises)
    .then(function(results) {
      ...
      /* Load RSVP counts for all events from subcollections */
      var countPromises = SM.events.map(function(ev) {
        return db.collection('communities').doc(ev.community)
          .collection('events').doc(ev.id)
          .collection('rsvps').get()
          .then(function(snap) {
            var counts = { going: 0, maybe: 0, notgoing: 0 };
            snap.forEach(function(d) {
              var s = d.data().state;
              if (counts.hasOwnProperty(s)) counts[s]++;
            });
            ev._goingCount    = counts.going;
            ev._maybeCount    = counts.maybe;
            ev._notgoingCount = counts.notgoing;
          }).catch(function() {
            ev._goingCount = ev._maybeCount = ev._notgoingCount = 0;
          });
      });
      return Promise.all(countPromises).then(function() { return SM.events; });
    })
    .catch(function(err) {
